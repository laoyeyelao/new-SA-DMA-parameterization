function Conc_new=Manual_Dynamics(TI,Conc,Gamma,CS,Dp_tot,Rel,Beta)
    if TI~=0
    Coll=Conc'*Conc.*Beta;
    ...
    for i=1:length(Conc_new)
        for j=1:i
            if ~isnan(Rel(i,j))
                    Conc_new(j)=Conc_new(j)+Gamma(i,j)*Conc_new(Rel(i,j))*F_evap(Rel(i,j))*TI/2;
                    Conc_new(i)=Conc_new(i)+Gamma(i,j)*Conc_new(Rel(i,j))*F_evap(Rel(i,j))*TI/2;
            end
        end
    end
    for i=1:length(Conc_new)
